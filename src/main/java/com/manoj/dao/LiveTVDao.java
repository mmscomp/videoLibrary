...
import com.manoj.service.LiveTVService;
@Repository
...
//	@Override
	public List<LiveTV> liveTVList() {
		System.out.println("2. ");
		Session session = this.sessionFactory.getCurrentSession();
		System.out.println("3. ");
		List<LiveTV> liveTVList = session.createQuery("from LiveTV").list();
		System.out.println(liveTVList);
		for(LiveTV m : liveTVList){
...
//	@Override
	public void removeLiveTV(int id) {
		Session session = this.sessionFactory.getCurrentSession();
		LiveTV l = (LiveTV) session.load(LiveTV.class, new Integer(id));
		if(null != l){
			session.delete(l);
		}
		logger.info("Live TV deleted successfully, movie details="+l);
	}
